...
const openImageLibrary = async () => {
  ...
  if (!result.canceled) {
    ...
    setImageName(result.assets[0].fileName || 'Selected Image');
  } else {
  ...
  useEffect(() => {
    validateForm();
  }, [name, lastName, email, motpasse, phone, mark, serie, imageUri]);
  ...
  const validateForm = () => {
    ...
    } else if (!onlyLettersRegex.test(mark)&&specialCharsRegex.test(mark)) {
      errors.mark = 'Mark must contain alphabetic letters only.';
    } 

    // Validate Serie (Password)
  if (!serie) {
    errors.serie = 'Password is required.';
  } 
...
{touched.country && errors.country && <Text style={styles.errorText}>{errors.country}</Text>}
        ...
        {touched.Serie && errors.serie && <Text style={styles.errorText}>{errors.serie}</Text>}

        {touched.Mark && errors.mark && <Text style={styles.errorText}>{errors.mark}</Text>}
